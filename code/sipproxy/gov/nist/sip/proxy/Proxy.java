...
import gov.nist.javax.sip.header.*;
		...
		*/
		...
		public class Proxy implements SipListener  {
		    ...
		    private ArrayList<ProxyCall> liveCalls;
		    ...
		    */
			...
			this.listeningPoints = new LinkedList();
		        if (confFile==null) {
				    ...
				    ("ERROR: the configuration file is not correct!"+
		                ...
		                liveCalls = new ArrayList<ProxyCall>();
		    		    observers = new ArrayList<Observer>();
		                Observer callStart = new CallStart(this);
		                Observer callEnd = new CallEnd(this);
		                attach(callStart);
		                attach(callEnd);
		            }
		            catch (Exception ex) {
		    ...
		    public void notify(Observer o, ProxyCall call) {
		    	o.update(call);
		    }
		...
		/*****************************************************************************/            
				...
				return;
		           ...
		           FromHeader fromUser = (FromHeader) clonedRequest.getHeader(FromHeader.NAME);
		           fromUser.removeParameter("tag");
		           System.out.println("Header of source is " + fromUser.toString());
		           ToHeader toUser = (ToHeader) clonedRequest.getHeader(ToHeader.NAME);
		           String tag = toUser.getParameter("tag");
		           System.out.println("Header of target is " + toUser.toString());
		           toUser.removeParameter("tag");
		           
		           String callerURI = fromUser.getAddress().getURI().toString();
		           String calleeURI = toUser.getAddress().getURI().toString();
		           String call_id_h = ((CallIdHeader) request.getHeader(CallID.CALL_ID)).toString();
		           String call_id = call_id_h.substring(call_id_h.indexOf(" ")+1, call_id_h.indexOf("@")-1);
		           
		           String caller = callerURI.substring(callerURI.indexOf("sip:")+4,callerURI.indexOf("@"));
		           String callee = calleeURI.substring(calleeURI.indexOf("sip:")+4,calleeURI.indexOf("@"));
		           System.out.println("in process Request, caller=" + caller + " callee=" + callee);
			
		           ProxyCall call = new ProxyCall(call_id, caller, callee);
			
		           ForwardingServer forwardingServer = new ForwardingServer(call);
	        	   String finalCallee = forwardingServer.findFinalCallee();
	        	   
	        	   if (finalCallee != null) {
	        		   if (finalCallee != "") {
	        			   System.out.println("The call is forwarded to " + finalCallee);
	        			   System.out.println("Initial Header: ");
	        			   System.out.println(request.toString());
	        			   System.out.println("+++++++++++++++++++++");
	        			   String newCallee = "sip:" + finalCallee + calleeURI.substring(calleeURI.indexOf("@"));
	        			   // see  \SipCommunicator-Fall05\net\java\sip\communicator\sip\CallProcessing.java
	        			   // line 775 and below 
	        			   URI newRequestURI = addressFactory.createURI(newCallee);
	        			   Address toAddress = addressFactory.createAddress(newRequestURI);
					   	   ToHeader toHeaderFinal = headerFactory.createToHeader(toAddress, null);
					   	   toHeaderFinal.setParameter("tag",tag);
					   	   // API advises not to use setHeader for ToHeader but gets the job done ...
					   	   Request newRequest = (Request) request.clone();
					   	   newRequest.setHeader(toHeaderFinal);
					   	   //System.out.println(toHeaderFinal.toString());
		    ...
		    public void processResponse(ResponseEvent responseEvent) {
		        try{
		            ...
		            FromHeader fromUser;
		            ToHeader toUser;
		            String callerURI, calleeURI, caller, callee;
		            ProxyCall call = null;
			    ...
			    CSeqHeader cseqHeader = (CSeqHeader)response.getHeader(CSeqHeader.NAME); 
			     ...
			     if (cseqHeader.getMethod().equals("SUBSCRIBE")) {
				    ...
				    fromUser = (FromHeader) response.getHeader(FromHeader.NAME);
				    toUser = (ToHeader) response.getHeader(ToHeader.NAME);
				    String call_id_h = ((CallIdHeader) response.getHeader(CallID.CALL_ID)).toString();
			        String call_id = call_id_h.substring(call_id_h.indexOf(" ")+1, call_id_h.indexOf("@")-1);
				    //System.out.println("in process response, toUserHeader: " + toUser.toString());
				    callerURI = fromUser.getAddress().getURI().toString();
				    calleeURI = toUser.getAddress().getURI().toString();
				           
				    caller = callerURI.substring(callerURI.indexOf("sip:")+4,callerURI.indexOf("@"));
				    callee = calleeURI.substring(calleeURI.indexOf("sip:")+4,calleeURI.indexOf("@"));
				    System.out.println("in process response, caller=" + caller + " callee=" + callee);
				    	 
				    if (cseqHeader.getMethod().equals("INVITE") && response.getStatusCode()== 200) {
				    	 call = new ProxyCall(call_id, caller, callee);
				    	 liveCalls.add(call);
				    	 notify(observers.get(0), call);
				    	 System.out.println("Call between " + caller + " and " + callee + " started");
				    	 System.out.println("Start time: " + call.getT_start().toString());
				    }
				    else if (cseqHeader.getMethod().equals("BYE")) {
				    	for (ProxyCall liveCall : liveCalls) {
					    	if (liveCall.getCall_id().equals(call_id)) {
					    		call = liveCall;
					    		break;
					    	}
					     }
				    	 if (call != null) {
				    		 notify(observers.get(1), call);
				    		 System.out.println("Call between " + caller + " and " + callee + " ended");
				    		 System.out.println("End time: " + call.getT_end().toString());
				    		 liveCalls.remove(call);
				    		 System.out.println("Call removed from live calls");
				    	 }
				     }
			     }
